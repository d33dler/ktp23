  ...
  const handleChange = (value) => {
   ...
   switch (item.type) {
    ...
    case 'radio':
        ...
        <div className="mt-2">
            <Form.Label>{item.label}</Form.Label>
            <Form.Control as="radio" type="radio" value={currentValue} onChange={(e) => handleChange(e.target.value, item.value)}>
            {
                item.options.map((opt, index) => {
                    return (
                        <Form.Check type="radio" label={item.labels ? item.labels[index] : opt} value={opt} />
                    );
                })
            }
            </Form.Control>
